...
const wpmText = document.querySelector(".wpmText span")
...
function randomPara() {
    let n = Math.floor(Math.random() * paragraphs.length);
    paragraphs[n].split("").forEach(letter => {
        let span = `<span>${letter}</span>`;
        wordsContainer.innerHTML += span;
    })
    document.addEventListener("keydown", () => inputFeild.focus())
    wordsContainer.addEventListener("click", () => inputFeild.focus())
}
function startType() {
    if (gameOver === false && interval == null) {
        interval = setInterval(timer, 1000);
    }
    const characters = wordsContainer.querySelectorAll("span")
    let enterdChar = inputFeild.value.split("")[currentIndex];
    console.log(enterdChar)
    if (enterdChar == null) {
        currentIndex--;
        characters[currentIndex].classList.remove("correct", 'incorrect')
    } else {
        if (characters[currentIndex].innerText === enterdChar) {
...
inputFeild.addEventListener("input", startType)
randomPara()
...
function getTimerText() {
    ...
    if (seconds <= 0) {
        ...
        interval = null;
        gameOver = true;
        let wpm = Math.floor((currentIndex / 60) * 100);
